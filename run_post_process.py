...
# All parts of the script will use this configuration.
logging.basicConfig(
    level=logging.INFO,
...
def extract_cel_bounding_box(video_path, threshold_value=240, min_contour_area=100):
    # logger is now defined at the module level
...
def crop_video_with_bounding_box(input_video_path, output_video_path, bounding_box):
    ...
    if w <= 0 or h <= 0:
        logger.error(f"Bounding box has invalid dimensions. W:{w}, H:{h}")
        return False
    ...
    logger.info(f"Cropping video. Input: '{input_video_path}', Output: '{output_video_path}', Box: {bounding_box}")
    frames_processed = 0
    while True:
        ret, frame = cap.read()
        if not ret:
    ...
    logger.info(f"Successfully cropped video. Processed {frames_processed} frames.")
    cap.release()
    out.release()
    return True
...
if __name__ == '__main__':
    # Note: Logging is now configured at the module level (top of the script)
    # logger variable is also available from module level.
    ...
    # Consider adding more arguments for thresholds, target dimensions etc. for more flexibility
    ...
    try:
        ...
        if bounding_box:
            ...
            logger.info(f"--- Step 2: Cropping video to bounding box (temp file: {temp_cropped_video_file_path}) ---")
            success_crop = crop_video_with_bounding_box(input_file_path, temp_cropped_video_file_path, bounding_box)

            if success_crop:
                logger.info(
                    f"Video cropped to bounding box and saved to temporary file: '{temp_cropped_video_file_path}'")

                logger.info(
                    f"--- Step 3: Resizing (contain with white padding, align: {alignment_choice}) the cropped video ---")
                success_resize = resize_video_contain_with_padding(
                    temp_cropped_video_file_path,
                    final_output_file_path,
                    alignment=alignment_choice
                    # Default padding_color is white, target_width/height are 512
                )
                ...
                logger.error(f"Failed to crop video to bounding box. Skipping resize step.")
        else:
            logger.warning(
